...
namespace IK_STL{
	...
	private:
		typedef allocator<LI_ITER::node<T>> nodeAllocator;
		typedef LI_ITER::node<T> *nodePtr;
	public:
		typedef T value_type;
		typedef LI_ITER::listIterator<T> iterator;
		typedef LI_ITER::listIterator<const T> const_iterator;
	...
	typename list<T>::nodePtr list<T>::newNode(const T& val = T()) {
		...
		nodeAllocator::construct(res, LI_ITER::node<T>(val, nullptr, nullptr, this));
		return res;
	}
	template<class T>
	...
	typename list<T>::const_iterator list<T>::changeIteratorToConstIterator(iterator& it)const {
		using nodeP = LI_ITER::node<const T>*;
		auto temp = (list<const T>*const)this;
		auto ptr = it.p;
		LI_ITER::node<const T> node(ptr->data, (nodeP)(ptr->prev), (nodeP)(ptr->next), temp);
	...
	void list<T>::swap(list& x) {
		IK_STL::swap(head.p, x.head.p);
		IK_STL::swap(tail.p, x.tail.p);
	}
	template<class T>
